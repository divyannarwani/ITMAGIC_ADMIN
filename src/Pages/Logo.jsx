...
function Logo() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await fetch("https://itmagic-backend.onrender.com/api/v1/addHomePageLogo", {
        method: "POST",
        body: formData
      });
  ...
  const fetchTableData = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("https://itmagic-backend.onrender.com/api/v1/getHomeLogo");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        const response = await axios.delete(`https://itmagic-backend.onrender.com/api/v1/deleteLogo/${id}`);
  ...
  return (
    <>
      <div className='text-3xl px-14 pt-4 text-gray-900 text-opacity-50'>
         MANAGE HOME PAGE LOGO's 
      </div>
      <div>
        <div className='relative px-4 py-8 bg-white rounded-xl shadow-xl'>
              <hr />
          <section className="justify-self-start bg-white" >
              <h2 className="relative my-8 left-10 top-2 text-2xl font-bold text-gray-900 ">ADD NEW LOGO</h2>
              <div className="relative left-10 top-2 mx-auto max-w-2xl">
                  <form onSubmit={handleSubmit}>
                      <div className="flex gap-8 mb-12">
                          <div>
                              <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 ">Logo Name</label>
                              <input 
                                type="text" 
                  ...
                  <tbody>
                      {tableData.map((row, index) => (
                        <tr key={index} className="bg-white border-b hover:bg-gray-50">
                          ...
                          <td className="p-4">
                              <img src={row.logo} className="w-16 md:w-32 max-w-full max-h-full" alt="Apple Watch" />
                          </td>
